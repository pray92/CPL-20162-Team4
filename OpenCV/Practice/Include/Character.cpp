...
HRESULT CCharacter::Initialize(void)
{
	...
	m_vPos = D3DXVECTOR3(rand() % WINSIZEX, rand() % (WINSIZEY - 300) + 300, 0.f);
	m_vDestination = D3DXVECTOR3(rand() % (WINSIZEX - 283 / 2) + 283 / 2, rand() % (WINSIZEY - (283 / 2 + 300)) + 283 / 2 + 300, 0.f);	// Set destination
...
SCENE CCharacter::Progress(void)
{
	...
	if (GetDMagnitude(m_vPos, m_vDestination) < 10)
	{
		...
		m_vDestination = D3DXVECTOR3(rand() % (WINSIZEX - 283 / 2) + 283 / 2, rand() % (WINSIZEY - (283 / 2 + 300)) + 283 / 2 + 300, 0.f);
